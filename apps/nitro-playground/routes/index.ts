...
<head>
  ...
  <title>evlog - Nitro Playground</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .method.put { background: #431407; color: #fb923c; }
...
<body>
  ...
  <script>
    const endpoints = [
      { method: 'GET', path: '/api/test/success', desc: 'Fetch user profile with caching' },
      { method: 'POST', path: '/api/test/create', desc: 'Create item with validation', body: { name: 'test-item', type: 'widget' } },
      { method: 'PUT', path: '/api/test/update', desc: 'Update item (always fails 403)', body: { name: 'test-item', version: '2.0' } },
      { method: 'DELETE', path: '/api/test/delete', desc: 'Delete item with cache invalidation' },
    ...
    async function fire(ep, i) {
      ...
      try {
        const res = await fetch(ep.path, {
          method: ep.method,
          ...(ep.body ? { headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(ep.body) } : {}),
        })
        const json = await res.json()
        const ok = res.ok || res.status === 403

        status.textContent = res.status
        status.className = 'status visible ' + (ok ? 'ok' : 'err')
